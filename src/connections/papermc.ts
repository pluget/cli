...
async function getDownloadUrlForSpecificBuild(
  version: string,
  build: number
): Promise<string | null> {
  try {
    const response = await axios.get(
      `https://papermc.io/api/v2/projects/paper/versions/${version}/builds/${build}`
    );
    return (
      `https://papermc.io/api/v2/projects/paper/versions/${version}/builds/${build}/downloads/` +
      response.data.downloads.application.name
    );
  } catch (err) {
    return null;
  }
...
export { getDownloadUrl, getDownloadUrlForSpecificBuild };
