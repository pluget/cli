...
export default async function installServer(path: string) {
  ...
  dirs["modulesServerDir"] = { path: "./modules/server", dest: "./bin", files: [] };
  dirs["modulesPluginsDir"] = { path: "./modules/plugins", dest: "./plugins", files: [] };
  [dirs["configDir"].files, dirs["logsDir"].files, dirs["worldsDir"].files, dirs["modulesServerDir"].files, dirs["modulesPluginsDir"].files] = await Promise.all([
  ...
  for (const dir in dirs) {
    for (const file of dirs[dir].files) {
      symlinkPromises.push(fse.symlink(join('..', dirs[dir].path, file), resolve(path, './bin', file)));
    }
  };
